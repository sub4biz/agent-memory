...
Known surface differences (not behavioral): the TypeScript SDK exposes `client.auth` for API-key + OAuth management, whereas the Python SDK manages keys via the dashboard/REST (see xref:reference/authentication.adoc[Authentication]). Both SDKs share the same `shortTerm` / `longTerm` / `reasoning` / `query` / `ontology` surface; ontology `import` / `diff` / `migrate` are REST-only in both.
====
