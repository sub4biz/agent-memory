...
Built documentation is output to `docs/build/site/`.
...
** *Build Command*: `npm run build` (Antora)
** *Output Directory*: `build/site`
** *Install Command*: `npm install`
+
NOTE: These settings are already encoded in `docs/vercel.json` (`buildCommand`, `outputDirectory: build/site`, `installCommand`, plus `/docs` → `/` redirects and clean-URL rewrites), so a fresh import generally needs no manual overrides.
...
. Verify the build copies it to `build/site/style.css`
