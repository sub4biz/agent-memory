...
| The unit of isolation. A workspace is bound to your API key (production), or selected per-request via `workspace_id` → the `X-Workspace-Id` header (header-scoped deployments). Different key/workspace = different graph.
|===
