...
On **NAMS**, the shared boundary is the **workspace** (bound to your API key): every agent using the same workspace shares the entity graph, while different workspaces are fully isolated from one another. On **bolt**, the shared boundary is the Neo4j database. Either way, `user_identifier=` sub-divides users *within* that boundary. See xref:explanation/backends.adoc#tenancy[Tenancy & scoping].
...
But if an orchestrator wants cross-agent visibility for audit, it can query Cypher (Bolt) or the read-only Cypher endpoint (NAMS):
