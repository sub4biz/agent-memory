...
in production. The SDK makes user identity a first-class concept — a
`:User` node and per-user scoping logic you no longer have to roll
yourself.
...
The SDK's multi-tenancy surface covers user-scoped conversations,
traces, and preferences, including the multi-tenant guardrails
described above.
