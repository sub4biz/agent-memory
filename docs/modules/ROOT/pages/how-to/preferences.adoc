...
[NOTE]
====
The basics (add a preference, search by category) port cleanly to the
**TypeScript SDK against NAMS**. The advanced bits (per-user scoping via
`user_identifier=`, time-travel via `supersede_preference`, bi-temporal
`as_of` queries) are **Python+Bolt only** — see
xref:how-to/multi-tenancy.adoc#typescript-counterpart[TypeScript multi-tenancy counterpart].
====
...
|Add a preference
|`client.long_term.add_preference(category, preference)`
|`memory.longTerm.addPreference(category, preference, { context })`

|Search preferences
|`client.long_term.search_preferences(query, category=, limit=)`
|`memory.longTerm.searchPreferences(query, { category, limit })`

|Per-user-scoped preferences
|`add_preference(..., user_identifier=)`
|_Not directly. Use `createConversation({ userId })` + scope by conversation. See xref:how-to/multi-tenancy.adoc#typescript-counterpart[TS multi-tenancy]._

|Supersede a preference
|`supersede_preference(old_id, new_id)`
|_Not exposed. Add a new preference; rely on recency in retrieval._

|Time-travel (`as_of=`)
|`get_preferences_for(identifier, as_of=)`
|_Not exposed on NAMS. Use `client.query.cypher(...)` if you really need it._

|`applies_to=[EntityRef(...)]`
|`add_preference(..., applies_to=[...])`
|_Not exposed. Embed the relation in the preference text or in `context`._
|===
...
// Store a preference. `context` is optional metadata explaining the source.
const preference = await memory.longTerm.addPreference(
  "brand",
  "Prefers Nike brand for running shoes",
  { context: "Stated in conversation conv-2026-05-22" },
);
